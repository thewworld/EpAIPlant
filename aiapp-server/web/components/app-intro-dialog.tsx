...
export function AppIntroDialog({ open, onOpenChange, appId, appName, appDescription, appIcon }: AppIntroDialogProps) {
  ...
  useEffect(() => {
    // 获取应用图标
    const iconSrc = appIcon || getAppIconById(appId)
    if (iconSrc) {
      setIconUrl(svgToDataUrl(iconSrc))
    }
  }, [appId, appIcon])
        ...
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold flex items-center gap-2">
            <div className="w-10 h-10 rounded-md overflow-hidden flex-shrink-0">
              <img src={iconUrl || "/placeholder.svg"} alt={appName} className="w-full h-full object-cover" />
            </div>
            {appName}
          </DialogTitle>
        </DialogHeader>
